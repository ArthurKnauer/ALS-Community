...
void UALSCharacterAnimInstance::UpdateAimingValues(float DeltaSeconds)
{
	...
	if (!RotationMode.VelocityDirection())
	{
		...
		AimingValues.AimSweepTime = FMath::GetMappedRangeValueClamped(
			FVector2f{ -90.0f, 90.0f }, FVector2f{ 1.0f, 0.0f }, AimingValues.AimingAngle.Y);
	...
	else if (CharacterInformation.bHasMovementInput)
	{
		...
		Delta.Normalize();
		const float InterpTarget = FMath::GetMappedRangeValueClamped(
			FVector2f{ -180.0f, 180.0f }, FVector2f{ 0.0f, 1.0f }, Delta.Yaw);
...
void UALSCharacterAnimInstance::TurnInPlaceCheck(float DeltaSeconds)
{
	...
	const float ClampedAimAngle = FMath::GetMappedRangeValueClamped(
		FVector2f{ TurnInPlaceValues.TurnCheckMinAngle, 180.0f },
		FVector2f{ TurnInPlaceValues.MinAngleDelay, TurnInPlaceValues.MaxAngleDelay },
...
{
	InAir.bJumped = true;
	InAir.JumpPlayRate = FMath::GetMappedRangeValueClamped(
		FVector2f{ 0.0f, 600.0f }, FVector2f{ 1.2f, 1.5f }, CharacterInformation.Speed);
